...
function AlternateInfoTip() {
  return (
    <div className="alternate-info">
      <button type="button" className="info-icon" aria-describedby="alternate-info-tip" aria-label="What each alternate route means">
        <span aria-hidden="true">i</span>
      </button>
      <span id="alternate-info-tip" className="info-tooltip" role="tooltip">
        The direct alternate is a single great-circle arc from departure to destination. A “Via …” alternate bends that same route through a recorded interior waypoint, so its distance is never shorter than the direct path. Distances are descriptive only.
      </span>
    </div>
  );
}
