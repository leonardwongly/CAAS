...
function normalizeLeg(value: unknown, index: number): RouteLeg | undefined {
  ...
  return {
    ...
    from,
    to,
    ...(stringValue(value, "airway") ? { airway: stringValue(value, "airway") } : {}),
    ...(stringValue(value, "airwayType") ? { airwayType: stringValue(value, "airwayType") } : {}),
...
export async function fetchAlternate(flightId: OpaqueId, signal?: AbortSignal): Promise<AlternateRoute> {
  const payload = await request("/api/v1/routes/alternate", {
    method: "POST",
    ...(signal ? { signal } : {}),
    body: JSON.stringify({ flightId }),
  });
  const data = isRecord(payload) && isRecord(payload.data) ? payload.data : undefined;
  if (!data) throw new ApiError(502, "The alternate response is not an object.", "ALTERNATE_MALFORMED");
  const geometry = normalizeGeometry(data.geometry);
  if (!geometry) throw new ApiError(502, "The alternate response has no usable geometry.", "ALTERNATE_MALFORMED");
  return {
    flightId: stringValue(data, "flightId") ?? flightId,
    callsign: stringValue(data, "callsign") ?? "Unknown callsign",
    origin: stringValue(data, "origin") ?? "Unknown departure",
    destination: stringValue(data, "destination") ?? "Unknown destination",
    kind: stringValue(data, "kind") ?? "direct-great-circle",
    label: stringValue(data, "label") ?? "Direct (great-circle) alternate",
    geometry,
    distanceNm: finiteNumber(data, "distanceNm") ?? 0,
  };
}
