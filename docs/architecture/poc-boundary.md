...
The separate airway-name list remains counts-only and never enters any product-facing DTO, log, signature, diff, geometry, completeness, or route comparison. The recorded `airway`/`airwayType` carried on a flight-plan route element is a leg annotation only: it is surfaced on resolved route legs but never supplies coordinates, connectivity, or topology. Route graphics still draw from exact resolved waypoint/reference coordinates. Never infer topology from adjacency, list membership, name similarity, or route text, and do not claim airway-topology conformance unless a later authoritative contract proves the relation.
