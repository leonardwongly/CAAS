...
The secretless CI workflow (`.github/workflows/ci-secretless-validation.yml`) is configured with a standalone Semgrep SAST job: `semgrep scan --config auto --error --metrics=off --oss-only --disable-version-check` in a digest-pinned official container — no AppSec platform, no secrets, no findings upload, no phone-home, failing the run on any finding. The auto rule set is deliberately mutable (fetched from the Semgrep registry per run; the image digest is bumped on review). It is a configuration, not evidence: no CI-run Semgrep result is retained, and no application security test, dependency scan, image scan, CI secret scan, deployed auth negative test, or Azure security control is evidenced. Future `PG-03`/`PG-04` evidence must bind results to the exact commit/OCI subject, environment, policy/validator hashes, measurements, artifact hashes, and failure fallback. A passing command exit alone is insufficient.
