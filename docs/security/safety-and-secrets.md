...
The secretless CI workflows are configured with four security gates, all failing the run on findings, all in digest-pinned containers without secrets or phone-home:
- Semgrep SAST (`.github/workflows/ci-secretless-validation.yml`): `semgrep scan --config auto --error --metrics=off --oss-only --disable-version-check`; the auto rule set is deliberately mutable (fetched from the Semgrep registry per run).
- Dependency audit (same workflow): `pnpm audit --prod --json` against the frozen lockfile — the same invocation the blocked `SEC-PACKAGE-AUDIT` check prescribes; no `--ignore-registry-errors`.
- Secret scan (same workflow): offline gitleaks (`gitleaks dir . --exit-code=1 --redact`), rules embedded in the binary.
