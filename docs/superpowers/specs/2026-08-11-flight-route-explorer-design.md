...
Status: Local-first POC design independently reviewed with no unresolved P0/P1; confirmed live API and read-only Azure capability evidence retained; `PG-00` remains blocked only by an explicitly authorized exact-hash commit and later implementation authorization
Version: 1.2-rc4
Date: 2026-08-12
...
`AC-DEL-*` snapshot/staging criteria later in this document:
...
> **Non-normative legacy warning:** Sections 3 through 29 below preserve earlier
> architecture analysis and contain terms such as staging, snapshots, synthetic
> fixtures, publisher jobs, reviewer access, and production promotion. None of
> those challenge-path mechanisms or acceptance statements may be implemented
> or used to interpret the POC. For the challenge, Section 0 and the current
