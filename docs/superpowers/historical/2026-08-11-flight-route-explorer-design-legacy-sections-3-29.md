# Flight Route Explorer - System Design: Archived Historical Sections 3-29

> **HISTORICAL ARCHIVE - non-binding.** This file preserves, verbatim, the
> former Sections 3 through 29 of the system design
> (`docs/superpowers/specs/2026-08-11-flight-route-explorer-design.md`,
> version 1.2-rc4). Section 0 of that design is the binding challenge/POC
> contract and supersedes all of this material. None of the mechanisms, roles,
> or acceptance statements preserved here may be implemented for or used to
> interpret the POC: staging and promotion, snapshot publishers, shadow
> validators, Blob state and pointers, Container Apps Jobs, Service Bus,
> controllers, attestors, reviewer access, synthetic runtime/demo fallbacks,
> Leaflet/OpenStreetMap tile providers, or multi-environment production topology.

## Extraction provenance

- Source: system design version 1.2-rc4, lines 313-1895.
- Extraction date: 2026-08-13, documentation-reconciliation workstream.
- Fidelity: the body is preserved verbatim. Only references to Sections 30-32
  (which remain in the main design document) are annotated with "of the main
  design document", and the section-numbering note below was added.
- Section numbering is unchanged from the original, so the former Sections
  5.1, 7, 8, 16, 17, 18, 19, 23, 28, and 29 discussions appear under the same
  numbers here.
- Section 30 (public-production gate), Section 31 (maintenance/deprecation/
  decommission), and Section 32 (references) remain in the main design
  document; every "Section 30"/"Section 31" mention in this archive points
  there. "Section 24" and other intra-archive references are internal to this
  historical material.
...
Live, read-only requests were made on 2026-08-12 using the local `.env`
`apikey` value without printing or retaining the secret or raw records.
...
- `GET https://api.swimapisg.info/geopoints/list/airports`
- `GET https://api.swimapisg.info/geopoints/list/navaids`

All five authenticated requests returned `200`. Flight Plan declared JSON;
aeronautical lists declared `text/plain` while carrying JSON arrays of strings.
Only the printed HTTPS hostname and exact GET path families are allowed. Legacy
HTTP/IP targets are rejected. Runways and generic search/existence operations
are not required by the accepted POC contract.

### 6.2 Observed response and route bounds

| Dataset | Bytes | Records | Important facts |
|---|---:|---:|---|
| Flights | 222,298 | 115 | 89 callsign groups; 24 duplicated; maximum multiplicity 4. |
| Flights with structured route elements | - | 59 | 927 elements; maximum 39; every sequence unique and monotonic. |
| Airways | 66,243 | 9,319 | Name list only; no authoritative occurrence/leg semantics proved. |
| Fixes | 5,655,307 | 247,419 | All parsed/validated; 210,905 unique IDs; max multiplicity 160. |
| Airports | 288,573 | 13,175 | All parsed/validated; unique IDs; resolves 115/115 departures and 114/115 destinations. |
| NAVAIDs | 206,841 | 10,195 | All parsed/validated; 5,726 unique IDs; max multiplicity 32. |

Of 891 designated route occurrences, Fixes matched 577, NAVAIDs matched 243,
234 were NAVAID-only, 9 matched multiple datasets, and 80 remained unresolved.
Thirteen additional route elements supplied direct coordinates and 23 lacked a
position object. Strict resolution therefore uses Fixes and NAVAIDs for
intermediate identifiers and Airports for endpoints while preserving ambiguity
and visible gaps.

There were 67 endpoint pairs, 32 with repeated records and at most 7 current
candidates for one pair. The first 30-second bounded probe completed each
request in 149-1,143 ms. No pagination metadata or flight-response rate-limit
headers were observed. Throttling was not deliberately induced, so no quota is
inferred.

### 6.3 Airway decision and drift

Structured route elements reported `DIRECT`, `NAMED`, `SID`, and `STAR` airway
types and 889 airway values. Only 653 values appeared in the separate airway
list, 236 did not, and route-text adjacency was mixed. This is insufficient to
prove whether a value belongs to an occurrence, inbound leg, or outbound leg.
Airway values are therefore hidden and cannot affect resolution, geometry,
completeness, signatures, diffs, distance, or rank.

Observed upstream drift remains a runtime-validation concern:

- the OpenAPI schema uses `enRoute`; the response uses `enroute`;
- live responses contain fields and aliases beyond the weak schema;
- route coordinates can be strings; and
- reference-list media types do not describe their JSON bodies.

The application treats OpenAPI material as discovery information, not as the
runtime trust boundary. Unknown fields are discarded and malformed or ambiguous
...
- Runtime Zod schemas are the contract source of truth and generate the checked-in OpenAPI document. CI fails on an unreviewed OpenAPI diff. Compatible additions require optional fields; removing or changing meaning/type requires a new versioned route plus migration/deprecation approval under Section 31 of the main design document.
...
The auth configuration, tenant/audience IDs, app-role requirement, allowed assignments, and access expiry are retained as release evidence. Challenge teardown removes guest/service-principal role assignments and federated smoke credentials, disables external ingress, and then follows Section 31 of the main design document. Production may replace this with a different approved access architecture, but cannot weaken the data-use or direct-origin gates.
...
- `AC-PROD-DEL-01` Section 30 of the main design document is complete with named owners and retained approval evidence.
...
| Missing production ownership/SLO/DR | Unmanaged incident or prolonged outage | Section 30 of the main design document blocks production until owners, objectives, incidents, recovery, and cost controls are approved. | Operations owner | Before production access | Open; production blocker |
...
- Section 30 of the main design document is complete and current.
...
The service may be described as production-operational only when Section 30 of the main design document is complete. Challenge deployment evidence, a successful application rollback, or an Azure resource deployment does not substitute for that gate.
...
|---|---|---|---|---|
| `ADR-001` | TypeScript/pnpm monorepo with explicit package boundaries | Accepted | POC decision authority | Toolchain or repository-boundary change |
| `ADR-002` | React/Vite, Fastify, and Zod runtime/public contracts | Accepted | POC decision authority | Framework or public API-contract change |
| `ADR-003` | Strict exact resolution with Airports/NAVAIDs, hidden airway values, full-precision modeled distance, and `0.000001 NM` rank equality | Accepted | POC decision authority | Source-data, route, distance, or ranking change |
| `ADR-004` | Retained Blob snapshots; superseded by no database, bounded in-memory generations, and session-local drafts | Superseded | POC decision authority | Persistence, sharing, or stronger concurrency requirement |
| `ADR-005` | Dedicated publisher, shadow validation, and pointer promotion; replaced by the one-server bounded live adapter | Rejected | POC decision authority | Ingestion, schema compatibility, or recovery change |
| `ADR-006` | Container Apps/Jobs topology; superseded by one Container App and managed environment under Bicep with no Jobs | Superseded | POC decision authority | Region, availability, topology, or cost change |
| `ADR-007` | Build-once GitHub Actions/Azure OIDC exact-digest supply chain with direct POC deployment and no staging promotion | Accepted | POC decision authority | Identity, registry, signing, scanning, or deployment change |
| `ADR-008` | Reviewer-restricted challenge access; superseded by one allowed Entra user with no anonymous access | Superseded | POC decision authority | Audience, tenant, or redistribution decision |

Status is `proposed | accepted | superseded | rejected`. These dispositions
record the user's ratified POC decisions dated 2026-08-12. Their exact-hash Git
record remains pending explicit commit authorization; superseded/rejected legacy
detail remains production-only historical analysis where Section 0 says so.
