...
- Computed alternate route (2026-08-23, [ADR-0003](../adr/0003-airway-labels-and-direct-alternate.md)): `POST /api/v1/routes/alternate` returns the densified direct great-circle path between resolved departure/destination plus its full-precision distance, clearly labelled `direct-great-circle`; the source route is never mutated, and identifiers travel in POST bodies only, never URLs.
...
| 31 | Show the computed alternate route | "Show direct alternate" draws the densified great-circle path dashed and labelled `direct-great-circle`; the recorded route stays solid and unchanged | ☐ |
| 32 | Confirm source unchanged | After showing the alternate, the source route DTO (gaps, distance, signature, comparison, ordering) is unchanged; no flight identifier or token appears in any URL | ☐ |
